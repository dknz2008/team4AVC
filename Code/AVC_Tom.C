/*Cite Jack Moran, he helped me write out and understand what was needed as our current implementation was a cypher */
...
/* Char */
char internetProtocol[] = "130.24.96.123";
...
double pixelTotal;
double pixelAverage;
double Kp = 0.4;   //Proportional Value (Keep values in the tens)
...
int networking(){
    ...
    send_to_server(sendMessage);//Sends "Please" to the server
...
int lineFollow(){
	...
	while(true){           //Forever True until certain event is met
		...
		for(int i = 0; i < sample; i++){             /*This proccess         */
			pixelArray[i] = get_pixel(i*10, 120, 3); /*Finds the brightness  */
		...
		for(int i = 0; i < sample; i++){             /*Finds if pixel is brighter                                     */
			if(pixelArray[i]>pixelAverage){          /*Then the average, a negitive number means turn left            */
				kpEv += (i-sample/2);                /*Positive number means turn right                               */
			}
		}
